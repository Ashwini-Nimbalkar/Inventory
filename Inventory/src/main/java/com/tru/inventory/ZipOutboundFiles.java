...
public class ZipOutboundFiles extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		startPolicy.setRouteStartTime("* 50 16 * * ?");
		//startPolicy.setRouteSuspendTime("* 52 15 * * ?");
		startPolicy.setRouteStopTime("0 55 16 * * ?");
		from("{{cinAppointment.path}}").routeId("Inventory-ZipOutboundFiles-Route").routePolicy(startPolicy).noAutoStartup()
		//from("file:C://in/Sql-Files?recursive=true&delay=5000&synchronous=true").routeId("ZipOutboundFiles-Route").routePolicy(startPolicy).noAutoStartup()
		//.marshal().zipFile()
