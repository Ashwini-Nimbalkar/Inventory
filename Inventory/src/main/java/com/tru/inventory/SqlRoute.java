...
public class SqlRoute extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		startPolicy.setRouteStartTime("* 53 00 * * ?");
		// startPolicy.setRouteSuspendTime("* 52 15 * * ?");
		startPolicy.setRouteStopTime("0 59 00 * * ?");
		...
		from("{{sql.timer}}").routeId("Inventory-SqlRoute").routePolicy(startPolicy).noAutoStartup()
		.to("{{seq.query}}")
				...
				.setHeader("seq", simple("${body[STRDTESEQ]}"))
